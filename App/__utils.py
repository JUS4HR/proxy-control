...
SUBPROCESS_SILENT_INFO = subprocess.STARTUPINFO(dwFlags=subprocess.STARTF_USESHOWWINDOW, wShowWindow=subprocess.SW_HIDE)
...
def getSSID() -> str | None:
    try:
        bytes = subprocess.check_output(['netsh', 'wlan', 'show', 'interfaces'], startupinfo=SUBPROCESS_SILENT_INFO)
...
def enable_startup():
    if platform.system() == 'Windows':
        key = winreg.OpenKey(winreg.HKEY_CURRENT_USER, STARTUP_REG_ENTRY, 0, winreg.KEY_WRITE)
...
def check_startup() -> bool:
    if platform.system() == 'Windows':
        key = winreg.OpenKey(winreg.HKEY_CURRENT_USER, STARTUP_REG_ENTRY, 0, winreg.KEY_READ)
        try:
